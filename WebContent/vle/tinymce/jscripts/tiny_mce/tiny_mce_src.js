...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getSel : function() {
			/* hack for safari window not containing document */
			//var t = this, w = this.win;
			var t = this, w = (this.win ? (this.win.document ? this.win : window) : window);

			return w.getSelection ? w.getSelection() : w.document.selection;
		},

		getRng : function(w3c) {
			/* hack for safari window not containing document (all references to w was
			 * formerly t.win */
			var t = this, s, r, w = (t.win ? (t.win.document ? t.win : window) : window);
			...
			try {
				if (s = t.getSel())
					r = s.rangeCount > 0 ? s.getRangeAt(0) : (s.createRange ? s.createRange() : w.document.createRange());
			} catch (ex) {
			...
			if (!r)
				r = isIE ? w.document.body.createTextRange() : w.document.createRange();
...
if(typeof eventManager != 'undefined'){
	eventManager.fire('scriptLoaded', 'vle/tinymce/jscripts/tiny_mce/tiny_mce_src.js');
};