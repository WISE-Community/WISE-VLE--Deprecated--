...
View.prototype.createAudioManagerOnProjectLoad = function(){
	...
	this.audioManager = new AudioManager('false', this);
...
View.prototype.startAudioAfterRender = function(nodeId){
	if(!this.updateAudioOnRender && this.audioManager){
		try {
			...
			// do nothing
			//this.notificationManager.notify('error thrown startAudioAfterRender', 3);
		}
		...
		if (audios[a].elementId) {
			var success = function(t,x,o){
				if (t == 'success') {
					...
					o.notificationManager.notify('could not create audio. Is your filesystem write-able? Does it have the right directories, ie audio, where the audio will go?', 3);
				}
			};
			...
			this.connectionManager.request('POST',1,'/webapp/author/authorproject.html',{forward:'filemanager',projectId:'',command:'updateAudioFiles',param1:contentBase,param2:audioUrl,param3:audios[a].elementTextContent},success, this, failure);
		}
	}
};
