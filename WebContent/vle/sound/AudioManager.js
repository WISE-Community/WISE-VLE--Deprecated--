...
soundManager.onload = function() {
	...
	notificationManager.notify('soundmanager onload!', 4);
...
AudioManager.prototype.setCurrentNode = function(node) {
	notificationManager.notify('audiomanager.setCurrentNode, node.id:' + node.id
		...
		soundManager.stopAll(); // stop currently-playing audio
	...
	if (true) {
		...
		// alert('audiomanager.js, nodeaudioelements.length:' +
		// nodeAudioElements.length);
		if (nodeAudioElements.length > 0) {
			for ( var i = 0; i < nodeAudioElements.length; i++) {
				...
				if (i != nodeAudioElements.length - 1) {
					...
					// this audio is not the last audio for this node, so upon
					// finishing, it should move to the next audio in the
					// sequence.
							...
							.createSound( {
								...
								onplay : function() {
									...
									if (vle.getCurrentNode().type != null
											&& (vle.getCurrentNode().type == "HtmlNode" || vle
													.getCurrentNode().type == "OTBlueJ")) {
										//alert('about to call highlight');
										vle.contentPanel.highlightElement(
												this.elementId,
												"3px dotted #CC6633");
									}
								},
								whileplaying : function() {
									var playPauseAudioElement = document
								...
								onfinish : function() {
									vle.contentPanel.highlightElement(
											this.elementId, "0px");
									vle.getCurrentNode().playAudioNextAudio(
											this.elementId);
								}
							});
					...
					// upon finishing, remember to set currentnode.audio to be
					// the first audio for the step.
					// alert('audiomanager, else, i:' + i + ", elementId: " +
					// nodeAudioElement.getAttribute("elementId"));
					var audio = soundManager
							.createSound( {
								id : nodeAudioElement.id,
								url : nodeAudioElement.url,
...
AudioManager.prototype.playPauseStepAudio = function() {
	...
	} else if (vle.getCurrentNode().audios.length > 0) {
		vle.getCurrentNode().audios[0].play();
	}
}
...
scriptloader
		.scriptAvailable(scriptloader.baseUrl + "vle/sound/AudioManager.js");