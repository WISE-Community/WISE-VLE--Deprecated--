function NavigationPanel(rootNode, autostep, stepLevelNum) {
	...
	this.stepLevelNumbering = stepLevelNum;
	this.currentStepNum;
}
...
NavigationPanel.prototype.getNavigationHtml = function(node, eventType, depth, position) {
    ...
    if (node.children.length > 0) {
    	...
    	if (submenu && submenu.className) {
    		htmlSoFar += "<div class=\""+ submenu.className +"\" id=\"" + node.id + "_menu\"><span style=\"border-left:" + pxIndent + "px solid #6699FF\" onclick=\"myMenu.toggleMenu(document.getElementById('"+ node.id +"_menu'))\">"+node.getTitle()+"</span>";
    	} else {
    		htmlSoFar += "<div class=\""+ classString +"\" id=\"" + node.id + "_menu\"><span style=\"border-left:" + pxIndent + "px solid #6699FF\" onclick=\"myMenu.toggleMenu(document.getElementById('"+ node.id +"_menu'))\">"+node.getTitle()+"</span>";
    	}
		...
		if(this.autoStep){
			htmlSoFar += "<a style=\"border-left:" + pxIndent + "px solid #6699FF\" class=\"" + classString + "\" onclick=\"vle.renderNode('" + node.id + "');\" id=\"" + node.id + "_menu\">" + icon + "Step " + this.currentStepNum + ": " + position + " " + node.getTitle() + "</a>";
			this.currentStepNum ++;
		} else {
			htmlSoFar += "<a style=\"border-left:" + pxIndent + "px solid #6699FF\" class=\"" + classString + "\" onclick=\"vle.renderNode('" + node.id + "');\" id=\"" + node.id + "_menu\">" + icon + position + " " + node.getTitle() + "</a>";
		};
	}
    //alert(htmlSoFar);
	return htmlSoFar;
