...
function generateDD(){	
	YUI({combine: true, timeout: 10000}).use('dd-constrain', 'dd-proxy', 'dd-drop', function(Y) {
	    ...
	    //var lis = Y.Node.all('#ddTable tr ul li');
	    //lis.each(function(v,k){console.log('creating draggable');var dd = new Y.DD.Drag({node: v, target: {padding: '0 0 0 20'}}).plug(Y.Plugin.DDProxy,{moveOnEnd: false}).plug(Y.Plugin.DDConstrained, {constrain2node: '#ddTable'});});
	    //lis.each(function(v,k){console.log('creating draggable');var dd = new Y.DD.Drag({node: v, proxy: true, moveOnEnd: false, constrain2node: '#ddTable',  target:{padding: '0 0 0 20'}});});
	    
	    //var uls = Y.Node.all('#ddTable tr ul');
	    //uls.each(function(v,k){console.log('creating target');var tar = new Y.DD.Drop({node: v});});
...
function createDraggable(Y, el){
	var dd = new Y.DD.Drag({node: el, target: {padding: '0 0 0 20'}}).plug(Y.Plugin.DDProxy,{moveOnEnd: false}).plug(Y.Plugin.DDConstrained, {constrain2node: '#ddTable'}); 
