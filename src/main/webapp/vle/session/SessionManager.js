	...
	if (this.lastSuccessfulRequest != null) {
		if ((Date.parse(new Date()) - this.lastSuccessfulRequest) +this.sessionTimeoutCheckInterval*1.5 >= this.sessionTimeoutInterval) {
			...
			$('#sessionMessageDiv').dialog(
					{autoOpen:true, draggable:true, modal:true, title:'Session Timeout', width:400, position:['center','top'], zIndex:10000, buttons: {'STAY LOGGED IN!':renewSessionSubmit}, close:renewSessionClose}
			);
		} else {
