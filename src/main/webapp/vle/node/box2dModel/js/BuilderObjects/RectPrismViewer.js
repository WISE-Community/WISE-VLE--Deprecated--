(function (window)
{
	...
	p.blocksInViewer = function (o){
		var count = 0;
		for (var i = 0; i < this.blockArray.length; i++){
			if (this.blockArray2d[i] != null) count++;
		}
		return count;
	}
	...
	p.placeBlock = function (o, x, y)
	{
		...
		if (x_index >= 0 && x_index < this.width_units && y_index >= 0 && y_index < this.height_units)
		{
			...
			{
				console.log(this.currentHeight_units,o.height_units , this.currentHeight_units + o.height_units, this.height_units);
				...
				if (curheight <= this.height_units)
				{
					...
					var i, underCount = 0;
					if (o.placed) this.blockArray.splice(this.blockArray.indexOf(o),1);
	...
	p.clearBlock = function (o)
	{
		...
		o.correct = false;
		o.incorrect = false;
		this.blockArray.splice(this.blockArray.indexOf(o),1);
		this.removeChild(o);
		this.resetBlocks();
		o.highlightDefault();
		return true;
