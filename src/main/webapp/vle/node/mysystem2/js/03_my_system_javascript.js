...
if(a.trim().length==0){return}MySystem.store.setStudentStateDataHash(JSON.parse(a))
})},});MySystem.activityController=SC.ObjectController.create({});MySystem.nodePaletteController=SC.ArrayController.create({contentBinding:"MySystem.activityController.paletteItems"});
...
var o=f.length;var j=0;for(j=0;j<o;j++){a=f[j];e=MySystem.store.createRecord(MySystem.PaletteItem,{title:a.name,image:a.image,uuid:a.uuid},MySystem.Activity.newGuid("palette_item"));
c.get("paletteItems").pushObject(e)}var b=g.energy_types;o=b.length;var l="";var n=null;
for(j=0;j<o;j++){l=b[j];n=MySystem.store.createRecord(MySystem.EnergyType,{label:l.label,color:l.color,isEnabled:YES,uuid:l.uuid},MySystem.Activity.newGuid("energyType"));
c.get("energyTypes").pushObject(n)}var d=g.diagram_rules;o=d.length;var h=null;var m=null;
for(j=0;j<o;j++){m=d[j];h=MySystem.store.createRecord(MySystem.DiagramRule,m,MySystem.Activity.newGuid("diagramRule"));
...
MySystem.DiagramRule=SC.Record.extend({suggestion:SC.Record.attr(String),comparison:SC.Record.attr(String),number:SC.Record.attr(Number),type:SC.Record.attr(String),hasLink:SC.Record.attr(Boolean),linkDirection:SC.Record.attr(String),otherNodeType:SC.Record.attr(String),paletteItem:function(a){if(a=="node"){return null
}var c=SC.Query.local(MySystem.PaletteItem,"title = {title}",{title:a});var b=MySystem.store.find(c);
return b.objectAt(0)},check:function(a){var b=this.matches(a);switch(this.get("comparison")){case"more than":return b>this.get("number");
case"less than":return b<this.get("number");case"exactly":return b==this.get("number");
default:throw"Invalid comparison value for diagram rule."}},checkNode:function(b,a){if(b===null){return true
}else{return b.get("uuid")==a.get("nodeType")}},checkLink:function(b,c,a){return this.checkNode(c,b.get("startNode"))&&this.checkNode(a,b.get("endNode"))
},matches:function(b){var d=this.paletteItem(this.get("type")),e=0;if(this.get("hasLink")){var c=this.paletteItem(this.get("otherNodeType")),a=MySystem.store.find(MySystem.Link);
switch(this.get("linkDirection")){case"-->":a.forEach(function(f){if(f.isComplete()&&this.checkLink(f,d,c)){e++
}},this);break;case"<--":a.forEach(function(f){if(f.isComplete()&&this.checkLink(f,c,d)){e++
}},this);break;case"---":a.forEach(function(f){if(f.isComplete()&&(this.checkLink(f,d,c)||this.checkLink(f,c,d))){e++
}},this);break;default:throw"Invalid linkDirection value for diagram rule."}return e
}else{if(d===null){return b.get("length")}else{b.forEach(function(f){if(this.checkNode(d,f)){e++
}},this);return e}}}});sc_require("models/diagram_rule");MySystem.DiagramRule.FIXTURES=[{guid:"dr1",suggestion:"Diagram needs at least 3 nodes.",comparison:"more than",number:2,type:"node"},{guid:"dr2",suggestion:"At least one node must be Clay.",comparison:"more than",number:0,type:"Clay"},{guid:"dr3",suggestion:"At least one Clay node linked to a Hand node.",comparison:"more than",number:0,type:"Clay",hasLink:true,linkDirection:"-->",otherNodeType:"Hand"},{guid:"dr4",suggestion:"Diagram needs at least 2 links.",comparison:"more than",number:1,type:"node",hasLink:true,linkDirection:"---",otherNodeType:"node"}];
MySystem.EnergyType=SC.Record.extend({label:SC.Record.attr(String,{isRequired:YES,defaultValue:"Energy Flow"}),color:SC.Record.attr(String,{isRequired:YES}),isEnabled:SC.Record.attr(Boolean,{isRequired:YES,defaultValue:YES}),uuid:SC.Record.attr(String)});
sc_require("models/energy_type");MySystem.EnergyType.FIXTURES=[{guid:"et1",label:"infrared radiation",color:"#490A3D",isEnabled:YES,uuid:"4bc46070-b92e-11e0-a4dd-0800200c9a66"},{guid:"et2",label:"thermal energy",color:"#BD1550",isEnabled:YES,uuid:"4bc46071-b92e-11e0-a4dd-0800200c9a66"},{guid:"et3",label:"solar radiation",color:"#E97F02",isEnabled:YES,uuid:"4bc48780-b92e-11e0-a4dd-0800200c9a66"},{guid:"et4",label:"kinetic energy",color:"#F8CA00",isEnabled:YES,uuid:"4bc48781-b92e-11e0-a4dd-0800200c9a66"},{guid:"et5",label:"light energy",color:"#8A9B0F",isEnabled:YES,uuid:"4bc48782-b92e-11e0-a4dd-0800200c9a66"}];
MySystem.ImprovedRadioView=SC.FieldView.extend({classNames:["sc-radio-view"],value:null,layoutDirection:SC.LAYOUT_VERTICAL,escapeHTML:YES,items:[],itemTitleKey:null,itemValueKey:null,itemColorKey:null,itemIsEnabledKey:null,itemIconKey:null,displayItems:function(){var g=this.get("items"),d=this.get("localize"),q=this.get("itemTitleKey"),p=this.get("itemValueKey"),e=this.get("itemIsEnabledKey"),n=this.get("itemIconKey"),a=this.get("itemColorKey");
var f=[],i=(g)?g.get("length"):0;var o,j,m,l,c,k,h,b;for(l=0;l<i;l++){o=g.objectAt(l);
if(SC.typeOf(o)===SC.T_ARRAY){j=o[0];m=o[1]}else{if(o){if(q){j=o.get?o.get(q):o[q]
}else{j=(o.toString)?o.toString():null}if(p){m=o.get?o.get(p):o[p]}else{m=o}if(e){k=o.get?o.get(e):o[e]
}else{k=YES}if(n){h=o.get?o.get(n):o[n]}else{h=null}if(a){b=o.get?o.get(a):o[a]}else{b=null
}}else{j=m=h=null;k=NO}}if(d){j=j.loc()}f.push([j,m,k,h,b])}return f}.property("items","itemTitleKey","itemValueKey","itemIsEnabledKey","localize","itemIconKey","itemColorKey").cacheable(),itemsDidChange:function(){if(this._items){this._items.removeObserver("[]",this,this.itemContentDidChange)
...
},$input:function(){return this.$("input")},displayProperties:["value","displayItems"],render:function(b,h){var q,j,o,r,p,d,a,c,f,l,e,k=this.get("displayItems"),m=this.get("value"),i=SC.isArray(m);
b.addClass(this.get("layoutDirection"));if(i&&m.length<=0){m=m[0];i=NO}if(h){r=SC.guidFor(this);
p=k.length;for(j=0;j<p;j++){q=k[j];o=q[3];if(o){d=(o.indexOf("/")>=0)?o:SC.BLANK_IMAGE_URL;
a=(d===o)?"":o;o='<img src="%@" class="icon %@" alt="" />'.fmt(d,a)}else{o=""}e=this._getSelectionState(q,m,i,false);
c=(!q[2])||(!this.get("isEnabled"))?'disabled="disabled" ':"";f=this.escapeHTML?SC.RenderContext.escapeHTML(q[0]):q[0];
var n="";if(q[4]){n=' style="background-color: '+q[4]+';"'}var g=!i&&m===q[1]?'checked="checked"':"";
b.push('<label class="sc-radio-button ',e,'"',n,">");b.push('<input type="radio" value="',j,'" name="',r,'" ',c," ",g,"/>");
b.push('<span class="button"></span>');b.push('<span class="sc-button-label">',o,f,"</span></label>")
}this._field_setFieldValue(this.get("value"))}else{if(k.length!=this.$input().length){this.render(b,YES)
}else{this.$input().forEach(function(s){s=this.$(s);k.forEach(function(t){if(t[1]==s){q=t
}});if(q){s.attr("disabled",(!q[2])?"disabled":null);l=this._getSelectionState(q,m,i,true);
s.parent().setClass(l)}s=j=l=null},this)}}},_getSelectionState:function(d,f,a,b){var e,h,c;
...
}},mouseDown:function(a){var d=a.target;while(d){if(d.className&&d.className.indexOf("sc-radio-button")>-1){break
}var c=d.value;d=d.parentNode}if(!d){return NO}d=this.$(d);d.addClass("active");this._activeRadioButton=d;
var b=this.get("displayItems");var e=b[parseInt(c,0)];this.set("value",e?e[1]:this._mixedValue);
...
MySystem.Link=SC.Record.extend({color:SC.Record.attr(String),text:SC.Record.attr(String),energyType:SC.Record.attr(String),startNode:SC.Record.toOne("MySystem.Node",{inverse:"outLinks"}),endNode:SC.Record.toOne("MySystem.Node",{inverse:"inLinks"}),sentences:SC.Record.toMany("MySystem.StorySentence",{inverse:"links",isMaster:NO}),energyTypeObj:function(){var c=this.get("energyType");
if(SC.none(c)){return null}var b=SC.Query.local(MySystem.EnergyType,"uuid = {uuid}",{uuid:this.get("energyType")});
var a=this.get("store").find(b);if(a.get("length")<1){return null}else{return a.objectAt(0)
}}.property("energyType"),energyTypeObjDidChange:function(){var a=this.get("energyTypeObj");
if(SC.none(a)){return}this.set("color",a.get("color"))}.observes("energyTypeObj"),linkStyle:{lineStyle:LinkIt.VERTICAL_CURVED,width:6,color:"#00ff00",cap:LinkIt.ROUND,arrows:LinkIt.ARROW_END},label:{text:"label",fontSize:12,fontFamily:"sans-serif",fontStyle:"normal",backgroundColor:"#ffffff"},startTerminal:SC.Record.attr(String),endTerminal:SC.Record.attr(String),formFields:[Forms.FormView.row(MySystem.ImprovedRadioView,{layout:{width:160,height:85},fieldKey:"energyType",fieldLabel:"Energy Type:",itemsBinding:"MySystem.activityController.energyTypes",itemTitleKey:"label",itemValueKey:"uuid",itemColorKey:"color",itemIsEnabledKey:"isEnabled",layoutDirection:SC.LAYOUT_VERTICAL}),Forms.FormView.row(SC.TextFieldView,{fieldKey:"text",fieldLabel:"Label:"})],isDimmed:NO,init:function(){arguments.callee.base.apply(this,arguments)
...
}}];sc_require("models/link");MySystem.Link.FIXTURES=[{guid:"link1",text:"First link",startNode:"1",startTerminal:"a",endNode:"2",endTerminal:"b",color:"#BD1550",energyType:"4bc46071-b92e-11e0-a4dd-0800200c9a66"},{guid:"link2",text:"Second link",startNode:"1",startTerminal:"a",endNode:"3",endTerminal:"b",color:"#BD1550",energyType:"4bc46071-b92e-11e0-a4dd-0800200c9a66"}];
MySystem.Node=SC.Record.extend(LinkIt.Node,{image:SC.Record.attr(String),title:SC.Record.attr(String),transformer:SC.Record.attr(Boolean,{defaultValue:true}),toolTip:SC.Record.attr(String,{defaultValue:null}),nodeType:SC.Record.attr(String),outLinks:SC.Record.toMany("MySystem.Link",{inverse:"startNode",isMaster:YES}),inLinks:SC.Record.toMany("MySystem.Link",{inverse:"endNode",isMaster:YES}),position:SC.Record.attr(Object),sentences:SC.Record.toMany("MySystem.StorySentence",{inverse:"nodes",isMaster:NO}),transformations:SC.Record.toMany("MySystem.Transformation",{inverse:"node",isMaster:YES}),terminals:["a","b"],formFields:[Forms.FormView.row(SC.TextFieldView,{fieldKey:"image",fieldLabel:"Image:"}),Forms.FormView.row(SC.TextFieldView,{fieldKey:"title",fieldLabel:"Title:"})],inColorMap:[],outColorMap:[],links:function(){var e=[],d;
...
};sc_require("models/node");MySystem.Node.FIXTURES=[{guid:"1",image:"images/00_my_system_clay_red_tn.png",title:"A Node",transformer:false,inLinks:[],outLinks:["link1","link2"],position:{x:160,y:160},transformations:["trans1"],nodeType:"030454f0-b92c-11e0-a4dd-0800200c9a66"},{guid:"2",image:"images/00_my_system_hand_tn.png",title:"B Node",transformer:false,inLinks:["link1"],outLinks:[],position:{x:310,y:10},nodeType:"12553af0-b92c-11e0-a4dd-0800200c9a66"},{guid:"3",image:"images/00_my_system_lightbulb_tn.png",title:"Third Node",transformer:false,inLinks:["link2"],outLinks:[],position:{x:10,y:10},nodeType:"1e3f7650-b92c-11e0-a4dd-0800200c9a66"}];
MySystem.PaletteItem=SC.Record.extend({image:SC.Record.attr(String),title:SC.Record.attr(String),uuid:SC.Record.attr(String)});
sc_require("models/palette_item");MySystem.PaletteItem.FIXTURES=[{guid:"pi1",title:"Clay",image:"images/00_my_system_clay_red_tn.png",uuid:"030454f0-b92c-11e0-a4dd-0800200c9a66"},{guid:"pi2",title:"Hand",image:"images/00_my_system_hand_tn.png",uuid:"12553af0-b92c-11e0-a4dd-0800200c9a66"},{guid:"pi3",title:"Bulb",image:"images/00_my_system_lightbulb_tn.png",uuid:"1e3f7650-b92c-11e0-a4dd-0800200c9a66"}];
...
sc_require("models/story");MySystem.Story.FIXTURES=[{guid:1,storyHtml:"<p>Make a system diagram to help explain how <i>both</i> the sun and people's actions affect the Earth's climate.</p><ul><li>Where does energy come from?</li><li>How does energy move?</li><li>Where does energy go?</li><li>How does energy change?</li></ul>"}];
...
};sc_require("models/story_sentence");MySystem.StorySentence.FIXTURES=[{guid:"ss1",order:1,bodyText:"First..."},{guid:"ss2",order:2,bodyText:"Then..."},{guid:"ss3",order:3,bodyText:"Finally..."}];
...
}}),SENTENCE_OBJECT_LINKING_SETUP:Ki.State.plugin("MySystem.SENTENCE_OBJECT_LINKING_SETUP"),SENTENCE_OBJECT_LINKING:Ki.State.plugin("MySystem.SENTENCE_OBJECT_LINKING"),checkDiagramAgainstConstraints:function(){var c=MySystem.activityController.get("diagramRules"),b=MySystem.store.find(MySystem.Node),a=[];
...
var b=MySystem.Node.newGuid();c=MySystem.store.createRecord(MySystem.Node,{title:a.title,image:a.image,position:{x:a.x,y:a.y},guid:b,nodeType:a.nodeType},b);
...
SC.Drag.addDropTarget(e);var c=d.clientX-20;var b=d.clientY-this.layout.top-10;var f={event:d,source:a,ghost:NO,slideBack:NO,data:{title:this.get("content").get("title")||"title",image:this.get("content").get("image")||"image",uuid:this.get("content").get("uuid"),clickX:c,clickY:b}};
...
var c=b.location.y-this.get("frame").y-b.data.clickY;var a={title:b.data.title,image:b.data.image,x:d,y:c,nodeType:b.data.uuid};
...
MySystem.activityController.set("content",c);MySystem.updateFromDOM()})};
