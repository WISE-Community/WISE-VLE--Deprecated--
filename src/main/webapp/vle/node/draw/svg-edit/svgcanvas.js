	...
	var removeUnusedDefElems = function() {
		...
		for(var i=0; i<all_len; i++) {
			...
			for(var j = 0; j < alen; j++) {
				...
				if(ref){
					var strippedRef = ref.match(/^http.*(#.*)/i);
					if (strippedRef){
						ref = strippedRef[1];
					}
				}
	...
	var getMouseTarget = function(evt) {
			...
			mouse_target.id != "svgcanvas") 
		{
			while (mouse_target.nodeName != "foreignObject") {
				...
				if(mouse_target.id=="svg_editor"){ //wise4 fix
				...
				if(current_mode == "path") {
					...
					if (current_path_pts.length == 0) {
						...
						// else, create a new point, append to pts array, update path element
							...
							addPointGrip(index, mouse_x, mouse_y); // fix pointgrip position when canvas is zoomed (wise4), TODO: report
						}
						keep = true;
					}
					return {
...
if(typeof eventManager != 'undefined'){
	eventManager.fire('scriptLoaded', 'vle/node/draw/svg-edit/svgcanvas.js');
};